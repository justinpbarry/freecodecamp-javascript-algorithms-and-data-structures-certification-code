...
// averageRating. Note that the rating values are saved as strings in the object and need to be
...
let christopherNolanMovies = watchList.filter(function(movie) {
    return movie.Director == "Christopher Nolan";
});

let numOfMovies = christopherNolanMovies.length;

let cNolanMovieScores = christopherNolanMovies.map(function(movie) {
    return parseFloat(movie.imdbRating);
});

let totalIMBDRating = cNolanMovieScores.reduce(function(accumulator, currentValue) {
    return accumulator + currentValue;
});

var averageRating = totalIMBDRating / numOfMovies;
